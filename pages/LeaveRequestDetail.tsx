...
const LeaveRequestDetail: React.FC<Props> = ({ navigateTo, mode = 'approver', status = 'pending', id, onAction }) => {
  ...
  const themeClass = isApproved ? 'from-emerald-600 to-emerald-700' : isRejected ? 'from-red-600 to-rose-700' : 'from-blue-600 to-indigo-700';
    ...
    <div className="animate-slideIn bg-[#f2f3f5] min-h-screen pb-24 font-sans">
      <div className={`pt-12 pb-20 px-6 relative overflow-hidden bg-gradient-to-br transition-colors duration-500 ${themeClass}`}>
        ...
        <div className="relative z-10 flex items-center justify-between">
          <div>
            <h2 className="text-white text-2xl font-bold">{isApproved ? '审批已通过' : isRejected ? '申请已被驳回' : '审批中'}</h2>
            <p className="text-white/70 text-sm mt-1">{isApproved ? '该休假申请已完成归档' : isRejected ? '请查看驳回原因' : '等待部门经理审批...'}</p>
          </div>
          <span className="material-symbols-outlined !text-6xl text-white/20">
            {isApproved ? 'check_circle' : isRejected ? 'cancel' : 'calendar_month'}
          </span>
        </div>
      </div>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
          <div className="flex items-center gap-4 mb-6">
            <div className="size-12 rounded-full bg-slate-100 overflow-hidden border border-slate-100 shadow-inner">
              <img src="https://picsum.photos/100?random=2" className="w-full h-full object-cover" alt="User" />
            </div>
            <div>
              <h3 className="text-lg font-bold text-slate-900">{isInitiator ? '我自己' : '赵子涵'}</h3>
              <p className="text-xs text-slate-400">行政部 · 2023-11-20 09:15</p>
            </div>
          </div>
          
          <div className="space-y-4">
            <DetailRow label="请假类型" value="事假" valueClass="font-bold text-primary" />
            <DetailRow label="时长" value="2.0 天" valueClass="font-black text-lg" />
            <div className="pt-2 border-t border-slate-50">
              <p className="text-[10px] text-slate-400 mb-2 font-black uppercase tracking-widest">请假事由</p>
              <p className="text-sm text-slate-700 leading-relaxed font-medium">因家中紧急事务需请假处理两天，涉及水管破裂维修及相关保险理赔事宜。</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto p-4 bg-white/80 backdrop-blur-md border-t border-slate-100 z-50 flex gap-3 shadow-[0_-5px_20px_rgba(0,0,0,0.05)]">
          {!isInitiator ? (
            ...
            <button 
              ...
              className="flex-1 py-3.5 bg-primary text-white font-bold rounded-xl active:scale-[0.98] transition-all shadow-lg shadow-primary/20"
            >
              催办进度
            </button>
          )}
